...
namespace MultiSpeak4
{
    ...
    /// 
    /// Summary description for Web Services Hosted by Switching Orders(SWO). 
    /// </summary> 
    ///  
    ...
    public class SWO_Server : System.Web.Services.WebService
    {
        ...
        #region Methods for Getting Data from the SWO system

        [WebMethod(Description = "The Requester requests from the server a specific switchingOrder, identified by the switchingOrderID. ")]
        [SoapHeader("MultiSpeakMsgHeader", Direction = SoapHeaderDirection.InOut)]
        public switchingOrder GetSwitchingOrderByID(string soID)
        {
            return null;
        }

        [WebMethod(Description = "Returns switching orders for a date range.  The calling parameter lastReceived is included so that large sets of data can be returned in manageable blocks.  lastReceived should carry an empty string the first time in a session that this method is invoked.  When multiple calls to this method are required to obtain all of the data, the lastReceived should carry in subsequent calls the objectID of the data instance noted by the server as being the lastSent.")]
        [SoapHeader("MultiSpeakMsgHeader", Direction = SoapHeaderDirection.InOut)]
        public switchingOrder[] GetSwitchingOrdersByDateRange(System.DateTime startDate, System.DateTime endDate, string lastReceived)
        {
            return null;
        }
        ...
        #region Methods for Modification of SWO data by external system
        ...
        #region Methods for Publisher to publish Event Notification to SWO(The subscriber)
